...
function run_event_based_simulation(in:: LoadData.Input, SD:: LoadData.immutableSimulationData)
    ...
    #import AMDGPU
    # using AMDGPU
    println(JACCPreferences.backend)
    println("Using AMDGPU as back end")
    ...
    # JACCPreferences.backend();  # Print the backend
    # println("The current backend is: ", JACCPreferences.backend)
    ...
    for i in 1:10
        iter_start_time = time_ns()  # Start time for this iteration
        ...
        iter_end_time = time_ns()  # End time for this iteration
        iter_time_ms = (iter_end_time - iter_start_time) / 1_000_000  # Time in milliseconds
        println("Iteration ", i, " Time taken: ", iter_time_ms, " ms")
    end
