...
function run_minibude(wgsize::Int)
    start_time = time()
    cmd = `julia --project=Threaded src/Threaded.jl -w $wgsize --deck ../../../data/bm2`
    
    # Create a unique log file for each run
    log_file = "minibude_w$(wgsize)Threaded.log"
    
    println("Running miniBUDE with $wgsize wgsize")
    ...
    open(log_file, "w") do io
        ...
        while process_running(process)
            if (time() - start_time) > 150
                ...
                println("Timeout reached for w=$wgsize, deleting log file...")
                close(io)
                rm(log_file)
                return
            end
...
function main()
    ...
    # Run all work group sizes
    for w in wgsize
        try
            run_minibude(w)
            println("Completed run with w=$w")
        catch e
            println("Error running with w=$w: $e")
        end
    end
