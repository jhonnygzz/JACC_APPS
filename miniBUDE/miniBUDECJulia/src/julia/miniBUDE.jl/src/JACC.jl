...
function run(params::Params, deck::Deck)
    nposes::Int = size(deck.poses)[2]
    etotals = JACC.Array{Float32}(undef, nposes)
...
#     JACC.parallel_for(nposes) do i
#         etot = MArray{Tuple{params.ppwi},Float32}(undef)
#         transform = MArray{Tuple{params.ppwi,3},Vec4f32}(undef)
...
#             lpos = MArray{Tuple{params.ppwi},Vec3f32}(undef)
...
#                 for j = 1:params.ppwi
#                     @inbounds x::Float32 = lpos[j].x - p_atom.x
#                     @inbounds y::Float32 = lpos[j].y - p_atom.y
#                     @inbounds z::Float32 = lpos[j].z - p_atom.z
...
#                     #Calculate steric energy
#                     @inbounds etot[j] += (One - (distij * r_radij)) * (zone1 ? Two * Hardness : Zero)

#                     chrg_e::Float32 =
#                         chrg_init * ((zone1 ? One : (One - distbb * elcdst1)) * (distbb < elcdst ? One : Zero))
#                     neg_chrg_e::Float32 = -abs(chrg_e)
#                     chrg_e = type_E ? neg_chrg_e : chrg_e
#                     @inbounds etot[j] += chrg_e * Cnstnt

#                     coeff::Float32 = (One - (distbb * r_distdslv))
#                     dslv_e::Float32 = dslv_init * ((distbb < distdslv && phphb_nz) ? One : Zero)
#                     dslv_e *= (zone1 ? One : coeff)
#                     @inbounds etot[j] += dslv_e
#                 end
#             end
#         end

#         for j = 1:params.ppwi
#             etotals[i + (j-1)*nposes] = etot[j] * Half
#         end
#     end

#     (Array{Float32}(etotals), params.ppwi)
# end
