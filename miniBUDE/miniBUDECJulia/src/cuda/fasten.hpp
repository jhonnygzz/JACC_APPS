...
public:
  ...
  template <typename T> [[nodiscard]] static T *allocate(size_t size) {
    T *data = nullptr;
#if defined(MANAGED)
    checkError(cudaMallocManaged(&data, sizeof(T) * size));
#elif defined(PAGEFAULT)
    data = (T *)std::malloc(sizeof(T) * size);
#else
    checkError(cudaMalloc(&data, sizeof(T) * size));
#endif
    return data;
  }
  ...
  template <typename T> [[nodiscard]] static T *allocate(const std::vector<T> &xs) {
    ...
    checkError(cudaMemcpy(data, xs.data(), xs.size() * sizeof(T), cudaMemcpyHostToDevice));
    return data;
  }
  ...
  [[nodiscard]] bool compatible(const Params &p, size_t wgsize, size_t device) const {
    ...
    free(data);
    cudaDeviceProp props{};
    checkError(cudaGetDeviceProperties(&props, device));
